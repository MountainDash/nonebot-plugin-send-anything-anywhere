...
try:
    ...
    from nonebot.adapters.discord.adapter import User
    ...
    async def _image(i: Image, bot: BaseBot) -> MessageSegment:
        ...
        if isinstance(image, (str, Path)):
            ...
            if path.exists():
                try:
                    ...
                    raise Exception(f"Error reading local image: {str(e)}")
            else:
                if not path.suffix in [".jpg", ".jpeg", ".png", ".gif"]:
        ...
        return MessageSegment.attachment(content=img_bytes, file=i.data["name"] if i.data["name"] != "image" else "img.png")
