...
from nonebot.adapters.discord.api import User, MessageFlag, MessageType
...
def mock_discord_message_event(create: bool = True):
    ...
    event_args = {
        ...
        "to_me": False,
        "reply": None,
        "_message": [TextSegment(type="text", data={"text": "/test"})],
    }
...
assert_discord = partial(assert_ms, Bot, SupportedAdapters.discord, **discord_kwargs())
...
async def test_send(app: App):
    ...
    async with app.test_matcher(matcher) as ctx:
        ...
        ctx.should_call_api("create_message", **data)
...
async def test_send_to(app: App):
    ...
    async def process(event: Event, bot: Bot):
        await MessageFactory(Text("123")).send_to(
            TargetDiscordChannel(channel_id=event.channel_id), bot=bot
        )
